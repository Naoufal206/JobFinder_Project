...
const storageBaseUrl = `${
  ...
  "https://jobfinderproject-production.up.railway.app"
}/storage`;
...
function Profile() {
  ...
  const fetchApplicantActivity = useCallback(async (userData, token) => {
    ...
    try {
      ...
      // api.js contains the Railway baseURL.
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      // api.get("/api/user")
      //
      // This sends the request to Railway.
      ...
      /*
       * If the API fails, try using the user
       * information already stored in localStorage.
       */
      ...
      if (userStr) {
        try {
          ...
          console.error("Local user data error:", storageError);
  ...
  const updateProfile = async (e) => {
    ...
    try {
       ...
       *
       * https://jobfinderproject-production.up.railway.app/api/profile
       */
  ...
  const completionItems = useMemo(() => {
    ...
    return [
      {
        label: "Full name",
        complete: Boolean(currentProfile?.name?.trim()),
      },
      {
        label: "Email address",
        complete: Boolean(currentProfile?.email?.trim()),
      },
      {
        label: "Location",
        complete: Boolean(currentProfile?.location?.trim()),
      },
      {
        label: "Profile photo",
        complete: Boolean(
  ...
  const applicationStats = useMemo(() => {
    return {
      ...
      interviews: recentApplications.filter(
        (application) =>
          String(application.status).toLowerCase() ===
          "interview scheduled"
      ).length,

      accepted: recentApplications.filter(
        (application) =>
          String(application.status).toLowerCase() ===
          "accepted"
      ).length,
  ...
  const nextInterview = useMemo(() => {
    return recentApplications
      ...
      .sort(
        (left, right) =>
          new Date(left.interview_datetime) -
          new Date(right.interview_datetime)
      )[0];
  }, [recentApplications]);
  ...
  return (
    <div className="profile-page">
      <section className="profile-hero">
        ...
        <h1>
          Keep your profile ready for every hiring step
        </h1>

        <p className="profile-subtitle">
          Manage your account details, upload a polished
          profile photo, and keep track of profile readiness
          from one workspace.
        </p>
      </section>
            ...
            <div className="profile-completion-card">
              ...
              <p>
                {completionPercentage === 100
                  ...
                  : "Add the remaining details to strengthen your profile."}
              </p>
            </div>
          </div>
              ...
              <div className="profile-section-card">
                ...
                {profile.role === "applicant" ? (
                  <>
                    ...
                    {nextInterview && (
                      <div className="profile-next-step">
                        ...
                        <p>
                          {new Date(
                            nextInterview.interview_datetime
                          ).toLocaleDateString()}{" "}
                          at{" "}
                          {new Date(
                            nextInterview.interview_datetime
                    ...
                    <div className="profile-activity-list">
                      {activityLoading ? (
                        <p className="profile-activity-placeholder">
                          Loading application activity...
                        </p>
                      ) : recentApplications.length ===
                        0 ? (
                        <p className="profile-activity-placeholder">
                          No applications yet. Start
                          applying to track progress
                          here.
                        </p>
                      ) : (
                        recentApplications
                  ...
                  <div className="profile-admin-note">
                    <p>
                      Use your account to publish roles,
                      review applications, and send
                      interview or offer updates from
                      the admin dashboard.
                    </p>
